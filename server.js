...
app.get("/qr", async (req, res) => {
  if (!lastQR) {
    return res
      .status(404)
      .send("<h2 style='font-family: monospace'>❌ No QR available. Please refresh or wait...</h2>");
  }
  ...
  if (age > QR_TTL) {
    ...
    return res
      .status(410)
      .send("<h2 style='font-family: monospace'>⌛ QR expired. Refresh to get a new one.</h2>");
  }
  ...
  try {
    ...
    res.send(`
      <html>
        <head>
          <title>Scan QR - BAN-MD Ultimate</title>
        </head>
        <body style="display:flex;align-items:center;justify-content:center;height:100vh;background:#111;color:#fff;font-family:sans-serif">
          <div style="text-align:center">
            ...
            <img src="${qrImage}" style="width:300px;height:300px;border:8px solid #fff;border-radius:10px"/>
            <p>⏳ Valid for ~${Math.floor((QR_TTL - age) / 1000)}s</p>
          </div>
        </body>
      </html>
    `);
