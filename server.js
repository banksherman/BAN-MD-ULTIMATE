// BAN-MD Ultimate Bot Server - Final Version
...
async function startSock() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      ...
      try {
        await sock.sendMessage(jid, { text: `🤖 *BAN-MD Ultimate Connected!*\n\n✅ You are now logged in.\n🎵 Enjoy using the bot!` });
      } catch (err) {
...
app.get("/qr", async (req, res) => {
  ...
  if (!lastQR) return res.send("<h3>⏳ QR not ready yet. Refresh in 1-2s...</h3>");
  if (Date.now() - qrAt > QR_TTL) return res.send("<h3>⏳ QR expired. Refresh page for a new one.</h3>");

  try {
    const qrImage = await qrcode.toDataURL(lastQR);
    const pairCode = Math.floor(Math.random() * 90000000) + 10000000; // 8-digit
    res.send(`
      <html>
