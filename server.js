...
┃ !owner    → Owner info
  ...
  ytmp3: {
    description: "Download YouTube audio",
    execute: async (sock, from, body) => commands.song.execute(sock, from, body)
  },
  ...
  video: {
    description: "Download YouTube video",
    execute: async (sock, from, body) => commands.ytmp4.execute(sock, from, body)
  },
  ...
  sticker: {
    ...
    execute: async (sock, from, body, msg) => {
      try {
        ...
        const buffer = mediaMessage.imageMessage
          ? mediaMessage.imageMessage.data
          : mediaMessage.videoMessage.data;
...
app.get("/qr", async (req, res) => {
  ...
  if (Date.now() - qrAt > QR_TTL) { lastQR = null; return res.send("<h3>⏳ QR expired. Refresh page for a new one.</h3>"); }
