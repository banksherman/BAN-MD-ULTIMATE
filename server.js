...
async function startSock() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (qr) {
      ...
      setTimeout(() => {
        if (Date.now() - qrAt > 20000) lastQR = null;
      }, 22000);
    }
...
app.get("/qr", async (req, res) => {
  if (!lastQR) {
    return res.send("<h3>❌ No QR available yet. Restarting or waiting for QR...</h3>");
  }
    ...
    res.send(`
      <html>
        ...
        <body style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;">
          ...
          <p>Alternatively use <code>/pair?phone=2567XXXXXXX</code> to get an 8-digit pairing code.</p>
        </body>
      </html>
    `);
  } catch (err) {
...
// Pair Code (real WhatsApp pairing via Baileys)
app.get("/pair", async (req, res) => {
  try {
    if (!sock) {
      return res.status(503).send("❌ Socket not ready yet. Try again in a few seconds.");
    }
    if (sock.user) {
      return res.send("<h3>✅ Already paired & connected.</h3>");
    }
      ...
      return res.send(`
        <html>
          ...
          <body style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;">
            ...
            <p>Provide your phone (with country code, numbers only):</p>
            <form method="GET">
              <input name="phone" placeholder="2567XXXXXXXX" style="padding:10px;font-size:16px" />
              <button type="submit" style="padding:10px 16px;margin-left:8px;font-size:16px;">Get Code</button>
            </form>
            <p style="margin-top:12px;">Or scan QR at <a href="/qr">/qr</a></p>
          </body>
        </html>
      `);
    }
    ...
    if (!phone) {
      return res.status(400).send("❌ Invalid phone. Use digits only, e.g. 2567XXXXXXXX");
    }

    const code = await sock.requestPairingCode(phone);
    const pretty = (code || "").replace(/\s+/g, " ").trim();
    ...
    res.send(`
      <html>
        ...
        <body style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;">
          <h2>🔗 BAN-MD PAIRING</h2>
          <p>Open WhatsApp → <b>Linked Devices</b> → <b>Link with phone number / code</b></p>
          <p>Enter this pairing code:</p>
          <h1 style="font-size:48px;letter-spacing:3px;margin:10px 0;">${pretty}</h1>
          <p>If it expires, refresh this page to generate a new one.</p>
          <p style="margin-top:12px;">Alternatively, scan QR at <a href="/qr">/qr</a></p>
        </body>
      </html>
    `);
  } catch (err) {
    console.error("❌ Failed to request pairing code:", err);
    res.status(500).send("❌ Failed to request pairing code. Check logs.");
  }
});
