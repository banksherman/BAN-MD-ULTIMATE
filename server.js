...
let jid = null;
const QR_TTL = 20000;
const SESS_DIR = path.join(__dirname, "sessions"); // persistent folder
...
// -------------------- QR Page with 8-digit pair code --------------------
app.get("/qr", async (req, res) => {
  if (!sock) await startSock();

  if (!lastQR) return res.send("<h3>❌ QR not ready yet. Refresh in 2s...</h3>");
  if (Date.now() - qrAt > QR_TTL) { lastQR = null; return res.send("<h3>⏳ QR expired. Refresh page.</h3>"); }
