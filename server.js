...
app.get("/qr", async (req, res) => {
  if (!lastQR) {
    return res.send("<h3>❌ No QR available. Please wait...</h3>");
  }
...
// Pair Code (real Baileys request or fallback demo)
app.get("/pair", async (req, res) => {
  try {
    if (!sock) return res.send("❌ Socket not ready.");

    // Try real WhatsApp pairing
    if (sock.requestPairingCode) {
      try {
        const phone = req.query.phone || ""; // user can provide number ?phone=2567xxxxxxx
    ...
    // Fallback: generate random 8-digit demo code
